...
class ElfCarrier : CarrierAlgorithm {
    ...
    override fun embed(carrier: ByteArray, payload: ByteArray): ByteArray {
        require(carrier.size >= 4) { "Invalid ELF file" }
        require(carrier.copyOfRange(0, 4).contentEquals(ELF_MAGIC)) { "Invalid ELF signature" }
    ...
    override fun supportedType(): FileType = FileType.ELF
